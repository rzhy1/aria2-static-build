...
prepare_libxml2() {
  ...
  libxml2_filename="$(basename "${libxml2_latest_url}")"

  # 下载文件
  if [ ! -f "${DOWNLOADS_DIR}/${libxml2_filename}" ]; then
    retry wget -c -O "${DOWNLOADS_DIR}/${libxml2_filename}.part" "https://download.gnome.org/sources/libxml2/${libxml2_latest_url}"
