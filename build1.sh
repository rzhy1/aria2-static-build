...
build_aria2() {
  ...
  ./configure CC=x86_64-w64-mingw32-gcc CXX=x86_64-w64-mingw32-g++ --host="${CROSS_HOST}" --prefix="${CROSS_PREFIX}" --enable-static --disable-shared --enable-silent-rules ARIA2_STATIC=yes ${ARIA2_EXT_CONF}
  make -j$(nproc)
  make install
