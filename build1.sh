...
prepare_xz() {
  ...
  CFLAGS="-O2 -g0 -flto" CXXFLAGS="-O2 -g0 -flto" ./configure --host="${CROSS_HOST}" --prefix="${CROSS_PREFIX}" --enable-silent-rules --enable-static --disable-shared --disable-doc --enable-debug=no --disable-nls
  make -j$(nproc)
  make install
