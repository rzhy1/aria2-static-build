...
prepare_xz() {
  ...
  CFLAGS="-mtune=generic -O3 -g0 -flto" CXXFLAGS="-mtune=generic -O3 -g0 -flto" ./configure --host="${CROSS_HOST}" --prefix="${CROSS_PREFIX}" --enable-silent-rules --enable-static --disable-shared --disable-doc --enable-debug=no --disable-nls
  make -j$(nproc)
  make install
...
prepare_libxml2() {
  ...
  CFLAGS="-mtune=generic -O3 -g0 -flto" CXXFLAGS="-mtune=generic -O3 -g0 -flto" ./configure --host="${CROSS_HOST}" --prefix="${CROSS_PREFIX}" --enable-silent-rules --without-python --without-icu --enable-static --disable-shared
  make -j$(nproc)
  make install
