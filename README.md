...
1、aria2c.exe is generated by build.sh, and aria2c1.exe by build1.sh. The main difference is their dependencies: aria2c.exe uses zlib and libexpat with the latest release tarballs, while aria2c1.exe uses zlib-ng and libxml2 with the latest source code.

2、Both aria2c.exe and aria2c1.exe are enhanced versions. The minimum "piece-length" is 1K, and the minimum "max-connection-per-server" is 1024.

3、The optimal Aria2 configuration can be found at: https://raw.githubusercontent.com/P3TERX/aria2.conf/master/aria2.conf
